"""
push_arabart_to_hub.py — upload the fine-tuned AraBART summarizer to the HF Hub.
...
Then:
    python push_arabart_to_hub.py --user your-username

What this does:
    1. Creates (or reuses) the repo  https://huggingface.co/{user}/{repo}
    2. Uploads the entire `Summarizer/` folder (weights, tokenizer,
       sentencepiece model, generation_config, README.md model card)
...
DEFAULT_DIR  = PROJECT_DIR / "Summarizer"
DEFAULT_REPO = "arabart-xlsum-arabic"
...
def main():
    ...
    p.add_argument("--folder", default=str(DEFAULT_DIR),
                   help="Local folder to upload (default: ./Summarizer)")
    p.add_argument("--private", action="store_true",
                   help="Create the repo as private")
    p.add_argument("--commit", default="Initial upload: arabart-xlsum-arabic",
                   help="Commit message")
    args = p.parse_args()
    ...
    if not (folder / "README.md").exists():
        print(f"No README.md in {folder} — uploading without a model card.")
